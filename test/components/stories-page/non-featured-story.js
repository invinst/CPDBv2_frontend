...
describe('NonFeaturedStory component', function () {
  ...
  it('should render correct number wrapper class', function () {
    instance = renderIntoDocument(<NonFeaturedStory story={ story } layout={ { grids: 4, isDisplayImage: true } }/>);
    ...
    instance = renderIntoDocument(<NonFeaturedStory story={ story } layout={ { grids: 5, isDisplayImage: true } }/>);
    findRenderedDOMComponentWithClass(instance, 'pure-u-2-5');

    unmountComponentSuppressError(instance);

    instance = renderIntoDocument(<NonFeaturedStory story={ story } layout={ { grids: 4, isDisplayImage: false } }/>);
    findRenderedDOMComponentWithClass(instance, 'pure-u-1-4');

    unmountComponentSuppressError(instance);

    instance = renderIntoDocument(<NonFeaturedStory story={ story } layout={ { grids: 5, isDisplayImage: false } }/>);
  ...
  it('should trigger handleClick on click', function () {
    NonFeaturedStory.should.triggerCallbackWhenClick(
      'handleClick', 'article-small', { story: story, layout: { grids: 4, isDisplayImage: false } }, story
    );
  });
});
